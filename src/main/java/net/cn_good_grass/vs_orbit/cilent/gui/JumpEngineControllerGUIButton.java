...
public class JumpEngineControllerGUIButton {
    ...
    public static void handler(JumpEngineControllerGUIButton message, Supplier<NetworkEvent.Context> contextSupplier) { NetworkEvent.Context context = contextSupplier.get(); context.enqueueWork(() -> { Player entity = context.getSender(); int buttonID = message.buttonID; int x = message.x; int y = message.y; int z = message.z; HashMap<String, String> textstate = message.textstate; handleButtonAction(entity, buttonID, x, y, z, textstate); }); context.setPacketHandled(true);}
    public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z, HashMap<String, String> textstate) {
        for (Map.Entry<String, String> entry : textstate.entrySet()) {
            String key = entry.getKey();
            String value = entry.getValue();
            JumpEngineControllerGUIMenu.guistate.put(key, value);
        }
        if (!entity.level().hasChunkAt(new BlockPos(x, y, z))) return;
    ...
    public static HashMap<String, String> readTextState(FriendlyByteBuf buffer) { HashMap<String, String> map = new HashMap<>(); for (int i = 0; i < buffer.readInt(); i++) { String key = buffer.readComponent().getString(); String value = buffer.readComponent().getString(); map.put(key, value); } return map;}
}
